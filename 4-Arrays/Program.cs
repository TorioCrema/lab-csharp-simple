...
namespace Arrays
{
    class Program
    {
        ...
        public static Complex MaxModulus(Complex[] array)
        {
            ...
            {
                var current = array[i];
                if (current.Modulus.CompareTo(max.Modulus) > 0)
                {
                    max = current;
                }
            }
        ...
        public static Complex[] Clone(Complex[] array)
        {
            ...
            {
                var current = array[i];
                clone[i] = new Complex(current.Real, current.Imaginary);
            }
        ...
        public static Complex[] SortByPhase(Complex[] array)
        {
            ...
            for (var i = 0; i < clone.Length; i++)
            {
                for (var j = i - 1; j >= 0; j--)
                {
                    if (clone[j + 1].Phase.CompareTo(clone[j].Phase) < 0)
                    {
                        Complex temp = clone[j];
                        clone[j] = clone[j + 1];
                        clone[j + 1] = temp;
                    }
                }
